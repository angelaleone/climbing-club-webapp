...
//GET rides from ride events
app.get('/api/ride/:rideeventid', async (req, res: any) => {
  const { rideeventid } = req.query
  try {
    const result = await pool.query('SELECT * FROM "Ride" WHERE rideeventid = $1', [rideeventid])
    if (result.rows.length === 0) {
