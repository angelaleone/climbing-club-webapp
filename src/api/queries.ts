import pool from './something'; //Import the pool
import * as dotenv from 'dotenv';

dotenv.config();

// Create Account
export const createAccount = async (admin_user: string, first_name: string, last_name: string, phone: string, ilstu_email: string, username_ilstu: string, password: string) => {
    try {
...
// Update Account
export const updateAccount = async (id: string, admin_user: string, first_name: string, last_name: string, phone: string, ilstu_email: string, username_ilstu: string, password: string) => {
    try {
...
export const deleteAccount = async (id: string) => {
    try {
        const deletedAccount = await pool.query('DELETE FROM "Account" WHERE accountID = $1 RETURNING *', [id]);
